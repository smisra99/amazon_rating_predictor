...
def plot_cm(y_true, y_pred, figsize=(15,15)):
    ...
    sfig.savefig('AdaBoost_1-5_CM.png')
def convert_dtype(x):
    if not x:
        return 0
    try:
...
df['reviewText_word_count'] = df["FinalReviewText"].apply(lambda x: len(str(x).split(' ')))
file_name = "AdaBoost_6-10_maxdepth-" + str(dtree_max_depth) + "_numtree-" + str(n_estimators)+".txt"
df = df.loc[(df['reviewText_word_count'] >= 6) & (df['reviewText_word_count'] <= 10)]
df = df.drop('reviewText_word_count', 1)
